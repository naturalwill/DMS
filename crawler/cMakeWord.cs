...
namespace crawler
{
    public class cWord
    {
        public string DocTitle, URL, tFilePath, pFilePath, type, rlDate, rlUnit;
        public cWord(string fileName, string _URL, string _type = "", string _rlDate = "", string _rlUnit = "")
        {
            DocTitle = fileName;
            URL = _URL;
            type = _type;
    ...
    public class cMakeWord
    {
        ...
        public void makeWord()
        {
            ...
            for (int i = 0; i < listWord.Count; i++)
            {
                ...
                cAccess.add(listWord[i].DocTitle, listWord[i].URL, listWord[i].pFilePath, listWord[i].type, listWord[i].rlDate, listWord[i].rlUnit,"批量添加的文档");
            }
